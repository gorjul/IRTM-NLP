...
from twitter_api_keys import myAccount
...
def clean_tweet(tw):
    ...
    """
    return ' '.join(re.sub("(@[A-Za-z0-9]+)|([^0-9A-Za-z \t])|(\w+:\/\/\S+)", " ", tw).split())


def analise_sentiment(tw):
    """
    Utility function to classify the polarity of a tweet
    using textblob.
    """
    analysis = TextBlob(clean_tweet(tw))
...
data['SA'] = np.array([analise_sentiment(tweet) for tweet in data['Tweets']])
